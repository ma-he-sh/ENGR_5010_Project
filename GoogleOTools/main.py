...
def get_city_distance(citydata, cityIndex1, cityIndex2 ):
    distance = citydata.loc[cityIndex1][cityIndex2]
    return distance

def get_normalized_demands( deliveryDemands ):
    minVal = min( deliveryDemands )
    maxVal = max( deliveryDemands )
    
    temp = []
    for i in range( len(deliveryDemands) ):
        z =  int( (deliveryDemands[i] - minVal) / ( maxVal - minVal ) )
        temp.append(z)
    return temp
